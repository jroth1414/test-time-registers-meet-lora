...
> **Amended 2026-09-02 after the whole-branch review.** The code as merged uses standard per-matrix
> LoRA: `LoRALinear(base, r, alpha, out_slices: list[slice] | None, dropout)` keeps one independent
> `(A_g, B_g)` pair per contiguous output slice (`lora_A: (g, r, in)`, `lora_B: (g, width, r)`), so
> `{q,v}` on the fused qkv costs `4rC` per block like published LoRA, and `{q,k,v}` yields three
> groups. `qkv_out_index` became `qkv_out_slices`. The listings below show the original shared-`A`
> draft (3rC) and its `tr == 384` count; the merged tests expect 512. `apply_lora` scans all selected
> blocks before wrapping any; `lora_state_dict` also carries `out_index` buffers and
> `load_lora_state_dict` rejects non-LoRA keys and mismatched targets.
